...
public class JDinkPushInteractionHandler extends AbstractJDinkInteractionHandler {

	private static final Log log = LogFactory.getLog(JDinkPushInteractionHandler.class);
	...
	public boolean interact(JDinkContext context, JDinkSprite sprite) {
		...
		List<JDinkCollision> collisions = context.getCollisionTester().getCollisionsAt(
				JDinkCollisionTestType.WALK, r, sprite);
		for (JDinkCollision collision: collisions) {
			if (collision instanceof JDinkSpriteCollision) {
				...
				if (scriptInstance != null) {
					JDinkScriptFunction function = scriptInstance.getFunctionByName("push");
					if (function != null) {
						try {
							if (log.isInfoEnabled()) {
								log.info("pushing sprite " + otherSprite.getSpriteNumber());
							}
							if (scriptInstance.isActive()) {
								log.info("not calling push, script instance still active");
							} else {
								scriptInstance.callFunctionBackground(context, function, new Object[0]);
							}
						} catch (Throwable e) {
							log.error("failed to execute push method due to " + e, e);
						}
					}
					result = true;
				}
